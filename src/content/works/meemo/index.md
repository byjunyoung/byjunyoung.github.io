...
사용자가 작성한 종이 메모를 기기에 넣으면 디지털 이미지로 변환해 클라우드(Dropbox)에 바로 업로드합니다. 종이에 쓰는 방식을 유지하면서 디지털 환경에서 메모를 검색하고 다른 기기에서 활용할 수 있습니다.
...
스캔이 진행되는 동안 전면 디스크가 천천히 회전하고, 스캔이 끝나면 멈춥니다. 사용자는 이 움직임으로 스캔 중임을 확인하고, 메모를 쓰는 과정에 작은 즐거움을 더합니다.
...
메모를 기기에 넣으면 디지털화되고, 작업이 끝나면 메모가 배출되어 다시 쓰거나 보관할 수 있습니다. 아날로그 작업 흐름을 끊지 않으면서 디지털 환경으로 옮기는 과정입니다.
...
물리적 상호작용을 구현하기 위해 아두이노 기반으로 시스템을 설계했습니다. 사용자가 종이를 넣으면 적외선 센서가 이를 감지하고 서보 모터가 종이를 안쪽으로 옮기며, 스텝 모터로 디스크를 돌려 진행을 알립니다. 촬영할 때는 카메라 둘레의 12구 네오픽셀 링을 흰빛으로 켜 어두운 내부를 비춥니다. 이 물리적 상호작용은 아날로그 경험을 디지털 환경으로 연결하는 역할을 했습니다.

### 클라우드 연동 및 애플리케이션 개발

![카메라가 기기 안에서 찍은 원본, 원형으로 잘라낸 이미지, 클라우드에 올라간 손글씨 메모](./19.jpg)

클라우드 저장을 구현하기 위해 라즈베리파이와 아두이노를 연동한 데이터 통신 시스템을 개발했습니다. 아두이노가 종이를 감지했다고 알리면, 라즈베리파이가 카메라로 1024×768 사진을 찍어 가운데를 정사각형으로 자르고 둥근 메모 모양 마스크를 씌웁니다. 파일 이름에 촬영 시각을 넣어 Dropbox에 올립니다.
...
Fusion 360으로 제품 외관과 구조를 3D 모델링했습니다. 외관은 CNC 가공으로, 내부 구조 부품은 3D 프린팅으로 제작해 조립과 유지보수가 쉽도록 설계했습니다. 이를 통해 Meemo는 물리적 인터페이스와 기능 구조를 갖춘 프로토타입으로 완성됐습니다.
