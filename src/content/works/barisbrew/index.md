...
매장에 도착하기 전에 주문하고, 대기 순번과 예상 완성 시간을 앱에서 봅니다. 제조 현황은 대기·제조 중·완료·오류 상태를 같은 화면 구조로 보여 주고, 완료되면 픽업을 안내합니다.
...
LLM 기반 음성 주문의 프롬프트와 대화 흐름을 설계했습니다. 성격·환경·말투·목표·가드레일·도구를 정의한 프롬프트, 호출어("바리스"), 말 끼어들기, 세션 종료 규칙과 완료 화면까지 다뤘습니다. 키오스크에는 음성 주문 진입점을 두고, 듣는 중·생각 중·응답·오류 상태를 화면 가장자리 애니메이션으로 구분합니다. 매장 시범 운영에서 나온 문제(TTS 소리가 마이크로 되돌아가는 현상 등)는 체크리스트로 관리하며 업데이트했습니다.
