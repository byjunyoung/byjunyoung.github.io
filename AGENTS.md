...
- 언어 자동 전환: `src/layouts/Base.astro` 헤드 인라인 스크립트가 첫 방문을 브라우저 언어로 보내고, 배지 선택은 `localStorage['site-lang']`에 기억한다. 이 키는 이력서 사이트와 공유하므로 이름·의미를 바꾸면 이력서 `index.html`도 같이 고친다. 상세는 설계 문서 §11.
...
- 순서는 제목 → 미디어 → 글(2026-09-14 지시, 9/7의 "글 먼저"를 바꿈). 절·소절 첫머리에 그 글이 설명하는 미디어를 두고 설명은 바로 뒤에. 한 절(기존 소절 포함)의 미디어는 모두 글 앞에 연달아 두고, **미디어 사이에 문장을 끼우지 않는다**(9/7 규칙 유지 — 사진·문단을 번갈아 짝지었다가 "이미지 사이 1–2줄 텍스트는 구리다"로 되돌림, 2026-09-14). 요청 없는 소제목(`###`)·부제를 만들거나 굵은 첫 문장을 제목으로 올리지 않는다. 문단은 쪼개거나 새로 쓰지 않는다.
- 프레이머 시절 사진은 사용자가 선별한 것이라 빼지 않는다. 많아 보이면 소절로 나눠 배치로 푼다.
