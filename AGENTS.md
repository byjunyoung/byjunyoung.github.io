...
- 순서는 제목 → 미디어 → 글(2026-09-14 지시, 9/7의 "글 먼저"를 바꿈). 절·소절 첫머리에 그 글이 설명하는 미디어를 두고 설명은 바로 뒤에. 한 절에 주제가 다른 미디어가 여럿이면 "미디어 → 그 미디어를 설명하는 기존 문단"을 반복해 짝짓는다. 이때 요청 없는 소제목(`###`)·부제를 만들거나 굵은 첫 문장을 제목으로 올리지 않는다(2026-09-14 지시). 문단은 쪼개거나 새로 쓰지 않는다.
