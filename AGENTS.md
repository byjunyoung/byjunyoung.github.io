...
- 순서는 제목 → 미디어 → 글(2026-09-14 지시, 9/7의 "글 먼저"를 바꿈). 절·소절 첫머리에 그 글이 설명하는 미디어를 두고 설명은 바로 뒤에. 한 절에 주제가 다른 미디어가 여럿이면 굵은 첫 문장을 `###` 소절로 올려 미디어와 글을 짝짓는다(문장은 새로 쓰지 않는다). 미디어 블록 사이에 문장을 끼우지 않는 건 그대로.
